...
class ScreenController extends Controller
{
    ...
    public function getHeatData($heat_id)
    {
        ...
        if($heat->van_1)
        {
            $heat->van_1 = HeatStat::find($heat->van_1)->first();
        }
        if($heat->van_2)
        {
            $heat->van_2 = HeatStat::find($heat->van_2)->first();
        }
        if($heat->van_3)
        {
            $heat->van_3 = HeatStat::find($heat->van_3)->first();
        }
        if($heat->truck_1)
        {
            $heat->truck_1 = HeatStat::find($heat->truck_1)->fresh();
        }
        if($heat->truck_2)
        {
            $heat->truck_2 = HeatStat::find($heat->truck_2)->fresh();
        }
        if($heat->truck_3)
        {
            $heat->truck_3 = HeatStat::find($heat->truck_3)->fresh();
        }
